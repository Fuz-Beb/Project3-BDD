...
public class GestionProces
{
    ...
    public void terminer(TupleProces tupleProces, int decisionProces) throws Exception
    {
        try
        {
            ...
            if (!proces.compareDate(tupleProces.getId()))
            ...
            idJuge = proces.changeJugeStatut(tupleProces.getId());
    ...
    public void creer(TupleProces tupleProces) throws Exception
    {
        try
        {
            ...
            if (!juge.existe(tupleProces.getJuge_id()))
                throw new IFT287Exception("Le juge " + tupleProces.getJuge_id() + "n'existe pas.");
            if (!partie.existe(tupleProces.getPartieDefenderesse_id()))
                throw new IFT287Exception(
                        "La partie defenderesse " + tupleProces.getPartieDefenderesse_id() + "n'existe pas.");
            if (!partie.existe(tupleProces.getPartiePoursuivant_id()))
                throw new IFT287Exception(
            ...
            juge.changerDisponibilite(false, tupleProces.getJuge_id());
            cx.commit();
        }
        catch (Exception e)
        {
